...
public class RouteExample extends RouteBuilder {
    ...
    public void configure() throws Exception {
        from("direct:main")
                .filter().translate().wrap()
                .enrich("getCreditScore").enrich("getBanksList").multicast()
                .to("tr1").to("tr2").to("tr3");

        from("tr1").to("Bank1");

        from("Bank1").bean("Normalizer")
                .aggregate("Aggregator").filter().translate().wrap().to("result");
        from("tr2").to("Bank2");
        from("Bank2").bean("Normalizer").aggregate("Aggregator");
        from("tr3").to("Bank3");
        from("Bank3").bean("Normalizer").aggregate("Aggregator");
    }
}